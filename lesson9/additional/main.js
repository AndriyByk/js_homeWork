...
funcForObjectsInArray(usersList);

function funcForObjectsInArray (array) {
    let fatherDiv = document.createElement("div");
    fatherDiv.classList.add("objectDiv");
    document.body.appendChild(fatherDiv);

    for (let element of array) {
        let objectDiv = document.createElement("div");
        fatherDiv.appendChild(objectDiv);

        recForObjects(element, objectDiv);

        function recForObjects (element, path) {
            for (let elementKey in element) {
                let newPath = document.createElement("div");
                newPath.classList.add("dynamicClass");
                let elemValue = element[elementKey];
                if (typeof elemValue !== "object") {
                    newPath.innerText = `${elementKey}: ${elemValue}`;
